...
const ViewProject = () => {
  ...
  const [projectSnapshotsPreview, setProjectSnapshotsPreview] = useState([]);
    ...
    const getProject = async () => {
      await axios
        ...
        .then((res) => {
          ...
          setProjectSnapshots(
            res.data.project.projectSnapshots && res.data.project.projectSnapshots.map(snapshot => snapshot.url)
          );
          setProjectSnapshotsPreview(
            res.data.project.projectSnapshots && res.data.project.projectSnapshots.map(snapshot => snapshot.url)
          );
        })
        .catch((error) => {
                ...
                <div className="w-full sm:col-span-4">
                  ...
                  <div className="mt-2">
                    <div className="mt-4 flex flex-wrap gap-4">
                      {projectSnapshotsPreview.map((preview, index) => (
                        <img
                          key={index}
                          src={preview}
                          alt={`Snapshot ${index}`}
